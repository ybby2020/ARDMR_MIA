...
class NCC(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        # set window size
        win = [6] * ndims if self.win is None else self.win
...
class mul_NCC(torch.nn.Module):
    """
    Local (over window) normalized cross correlation loss.
    """
    def __init__(self, win=None):
        super(mul_NCC, self).__init__()
        self.win = win

    def forward(self, y_pred, y_true):

        N, C = y_true.size(0), y_true.size(1)
        total_loss = 0.0
        for c in range(C):

            I = y_true[:, c:c + 1, ...]  # Single channel of y_true
            J = y_pred[:, c:c + 1, ...]
...
#
#         mshift1 = torch.zeros(12, 1, 3, 3, 3).cuda()
#         mshift1_view = mshift1.view(-1)
#         mshift1_view[torch.arange(12).to(img.device) * 27 + idx_shift1[:, 0] * 9 + idx_shift1[:, 1] * 3 + idx_shift1[:, 2]] = 1
#         mshift1 = mshift1_view.view(12, 1, 3, 3, 3)
#
#         mshift2 = torch.zeros(12, 1, 3, 3, 3).cuda()
#         mshift2_view = mshift2.view(-1)
#         mshift2_view[torch.arange(12).to(img.device) * 27 + idx_shift2[:, 0] * 9 + idx_shift2[:, 1] * 3 + idx_shift2[:, 2]] = 1
#         mshift2 = mshift2_view.view(12, 1, 3, 3, 3)
#
#         rpad1 = nn.ReplicationPad3d(dilation)
#         rpad2 = nn.ReplicationPad3d(radius)
#
...
class LocalCrossCorrelationLoss(torch.nn.Module):
    def __init__(self, window_size=5):
        super(LocalCrossCorrelationLoss, self).__init__()
        self.window_size = window_size
        self.window = torch.ones(1, 1, window_size, window_size, window_size)

    def forward(self, I, J):
        # Ensure the window is on the same device as the images
        self.window = self.window.to(I.device)

        # Compute local mean
        mu_I = F.conv3d(I, self.window, padding=self.window_size // 2, groups=1) / (self.window_size ** 3)
        mu_J = F.conv3d(J, self.window, padding=self.window_size // 2, groups=1) / (self.window_size ** 3)

        # Compute local squared mean
        I_sq = I * I
        J_sq = J * J
        mu_I_sq = F.conv3d(I_sq, self.window, padding=self.window_size // 2, groups=1) / (self.window_size ** 3)
        mu_J_sq = F.conv3d(J_sq, self.window, padding=self.window_size // 2, groups=1) / (self.window_size ** 3)

        # Compute local cross correlation
        cross = F.conv3d(I * J, self.window, padding=self.window_size // 2, groups=1) / (self.window_size ** 3)

        # Compute normalized cross correlation
        numerator = cross - mu_I * mu_J
        denominator = torch.sqrt((mu_I_sq - mu_I * mu_I) * (mu_J_sq - mu_J * mu_J))

        # Avoid division by zero
        ncc = numerator / (denominator + 1e-5)

        # Return the mean NCC loss
        return -torch.mean(ncc)

class ZSA:
    def __init__(self,device):
        self.device = device
        self.per = lpips.LPIPS(net='vgg', verbose=False).to(self.device)
    def zsaPer_loss(self,real,fake):
        sum = 0
        for i in range(real.size(2)):
            sum += self.per(real[0, 0, i, :, :], fake[0, 0, i, :, :])
        return sum
